...
function decryptPath(encryptedPath, key) {
    ...
    const encryptedBytes = hexToBytes(encrypt);
    // Step 2: Decrypt byte array
    const decryptedBytes = xorDecrypt(encryptedBytes, key);
...
function decryptSwappedPairs(str) {
...
function rotateArray(path, method, direction = "left") {
    ...
    const rotatedStr = str.substring(length - actualPositions) +
        str.substring(0, length - actualPositions);
    return task + "_" + rotatedStr;
}
function decryptMessagePack(path, method) {
    ...
    const originalPositions = msgpack.decode(originalPositionsBuffer);
    const scrambledPositions = [];
    for (let i = 0; i < originalPositions.length; i++) {
        scrambledPositions[originalPositions[i]] = i;
    }
    const decryptedPathChars = [];
...
async function decodeSHA6(path) {
    const { task, encrypt } = stringSplit(path);
    return task + "_" + encrypt;
}
const encryptionMethod = async (method, path) => {
    switch (true) {
        ...
        case method.includes("hex decoded, encrypted with XOR, hex encoded again. key: secret"):
...
async function start(endpoint) {
    try {
        ...
        if (response && response.encrypted_path && response.encryption_method) {
            ...
            const endpoint = await encryptionMethod(response.encryption_method, response.encrypted_path);
            start(endpoint);
        }
    }
    catch (err) {
